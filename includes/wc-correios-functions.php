...
function wc_correios_update_tracking_code( $order_id, $tracking_code ) {
	...
	if ( '' !== $tracking_code && $tracking_code !== $current ) {
		...
		//Variavel para o link do rastreador
		$rastreador = "<a href='http://websro.correios.com.br/sro_bin/txect01$.QueryList?P_LINGUA=001&P_TIPO=001&P_COD_UNI=$tracking_code' target='_blank'>$tracking_code</a>";
		
		// Add order note.
		$order->add_order_note( sprintf( __( 'Added a Correios tracking code: %s', 'woocommerce-correios' ), $rastreador ) );
