...
class WC_Correios_Webservice_International
{
    ...
    public function get_shipping()
    {
        ...
        if (is_wp_error($response)) {
            do_action('woocommerce_correios_international_shipping_error_response', $response);
            if ('yes' === $this->debug) {
            ...
            if (isset($result->tipo_servico)) {
				...
				do_action('woocommerce_correios_international_shipping_ws_response', $response);

                $shipping = $result->tipo_servico;
            }
        } else {
            do_action('woocommerce_correios_international_shipping_error_ws_response', $response);
            if ('yes' === $this->debug) {
